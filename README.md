...
- `GET /`
  ...
  - `{"app":"Progulka`s Dark and Darker game proxy","version":"1.0.0","details":"https://cadiastands.ru"}`
- `GET /dc/helloWorld`
...
5. Proxy поднимает TCP/UDP туннель на локальном порту из `DAD_PROXY_PORTS_RANGE`:
...
7. Трафик от пользователя до игровых серверов идет через поднятый туннель.
